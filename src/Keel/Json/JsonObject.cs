...
namespace Keel.Json
{
    public class JsonObject : JsonObjectModel// : IComparable, IConvertible, IComparable<bool>, IEquatable<bool>
    {
        ...
        public static explicit operator string(JsonObject ret)
        {
            return ret.ToString();
        }
        ...
        /// <summary>
        /// 获得1970年 1月 1日 N点, 0, 0以来的秒数，php中一般用此方法来表示时间
        /// N这个时间是使用当前时区的小时数减去utc当前的小时数
        /// </summary>
        /// <returns></returns>
        public static string GetJsonTime()
        {
            return Math.Floor((DateTime.Now - new DateTime(1970, 1, 1, DateTime.Now.Hour -DateTime.UtcNow.Hour , 0, 0)).TotalSeconds).ToString();
        }
        public static string TimeToJson(DateTime dt)
        {
            return Math.Floor((dt - new DateTime(1970, 1, 1, dt.Hour - dt.ToUniversalTime().Hour, 0, 0)).TotalSeconds).ToString();
        }
        public static DateTime  TimeFromJson(string  json)
        {
            int  l = int.Parse(json);
            TimeSpan ts=new TimeSpan(0,0,l );
            DateTime dt = new DateTime(); ;// DateTime.MinValue  +new DateTime(1970, 1, 1, dt.Hour - dt.ToUniversalTime().Hour, 0, 0)
            return  dt ;
            //new DateTime(1970, 1, 1, dt.Hour - dt.ToUniversalTime().Hour, 0, 0)).ToString();
        }
        /// <summary>
        /// 转换json中的unicode 字符串为汉字
        /// </summary>
